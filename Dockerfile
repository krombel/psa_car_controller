...
COPY . .
RUN poetry version ${PSACC_VERSION} && poetry build
#COPY ./dist/psa_car_controller-${PSACC_VERSION}-py3-none-any.whl .
RUN pip3 install --break-system-packages --no-cache-dir ./dist/psa_car_controller-${PSACC_VERSION}-py3-none-any.whl
EXPOSE 5000
...
RUN  apt update && apt-get install -y --no-install-recommends $PYTHON_DEP curl && \
     apt-get clean ; \
